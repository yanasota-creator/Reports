...
光電管を用いて光電効果の実験を行い、赤色（$\lambda=635\ \text{nm}$）・緑色（$\lambda=532\ \text{nm}$）・青色（$\lambda=450\ \text{nm}$）の単色光について、光電流が0となる限界電圧$V_0$を測定した。振動数$\nu$と$eV_0$の関係を最小二乗法で直線回帰したところ、プランク定数$h_{実験}=4.546\times10^{-34}\ \text{J·s}$を得た。これは文献値$h=6.626\times10^{-34}\ \text{J·s}$に対して誤差31.4%と大きく乖離しており、光電面材料の仕事関数は$W=1.306\times10^{-19}\ \text{J}$（0.816eV）と求まった。誤差の主要因として、使用した測定点が3点のみと少ないこと、メートルブリッジによる電圧較正の系統誤差の可能性に加え、カーテン上部の隙間から漏れた外光が光電流に混入した可能性を考察した。
...
- 単色光源：赤色（$\lambda=635\ \text{nm}$）・緑色（$\lambda=532\ \text{nm}$）・青色（$\lambda=450\ \text{nm}$）の3色
...
新物理学実験<sup>[2]</sup>（pp.170-179）参照。

## 6. 実験結果

### 6.1 検流計の読み－電圧特性

赤・緑・青の各色について、メートルブリッジの位置（距離）から決定した電圧と、そのときの検流計の読みとの関係を測定した。測定データの一部を表1に示す（全データはdata/planck_constant.csvを参照）。3色を比較しやすいよう、最下行を読みが0となった測定点（限界電圧$V_0$）に揃え、そこから何点手前の測定かを左端に示した。青色は$V_0$の4点前に相当する測定点がないため、この欄を「－」とした。

**表1　検流計の読み－電圧特性（抜粋、限界電圧$V_0$付近）**

| 測定点 | 赤：電圧 [V] | 赤：読み | 緑：電圧 [V] | 緑：読み | 青：電圧 [V] | 青：読み |
|:---|---:|---:|---:|---:|---:|---:|
| 4点前 | 0.406 | 3 | 0.784 | 3 | － | － |
| 3点前 | 0.434 | 2 | 0.812 | 1 | 1.022 | 10 |
| 2点前 | 0.462 | 1 | 0.826 | 1 | 1.036 | 4 |
| 1点前 | 0.476 | 1 | 0.840 | 0.2 | 1.0416 | 1 |
| $V_0$ | 0.490 | 0 | 0.854 | 0 | 1.0458 | 0 |
...
プランク定数の文献値<sup>[1]</sup>は$h=6.626\times10^{-34}\ \text{J·s}$である。本実験で得られた$h_{実験}=4.546\times10^{-34}\ \text{J·s}$との誤差は$\frac{4.546\times10^{-34}-6.626\times10^{-34}}{6.626\times10^{-34}}\times100\approx-31.4\%$であり、絶対値・符号の傾向（光電子のエネルギー効率が理論より低く見積もられている）を含めてかなり大きな系統的なずれが生じた。
